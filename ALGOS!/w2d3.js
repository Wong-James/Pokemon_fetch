...
function symmetricDifferences(numsA, numsB) {
  ...
  while (idxA < numsA.length && idxB < numsB.length) {
    if ([numsA[idxA]] !== [numsB[idxB]]) {
      unique.push(numsA[idxA])
      idxB++
  ...
  function symmetricDifferences2(numsA, numsB) {
    const result = []
    var i = 0
    var j = 0
    var check = false
    for (i = 0; i < numsA.length; i++) {
      //reset the check and j to 0
      j = 0
      check = false
  ...
  console.log(symmetricDifferences2(test1NumsA, test1NumsB));


  function symmetricDifferencesOptimal(numsA, numsB) {
    var unique = {}
    var idxA = 0;
    var idxB = 0;
